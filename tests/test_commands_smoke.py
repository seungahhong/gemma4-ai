...
def test_review_file_target(tmp_path: Path, mlx_stub: MLXStub) -> None:
    file = tmp_path / "foo.py"
    file.write_text("def add(a, b):\n    return a+b\n")
    mlx_stub.response = "## 요약\n괜찮습니다."
    runner = CliRunner()
...
def test_ask_one_shot_writes_session(tmp_home: Path, mlx_stub: MLXStub) -> None:
    mlx_stub.response = "안녕하세요"
    runner = CliRunner()
...
def test_ask_resume_last(tmp_home: Path, mlx_stub: MLXStub) -> None:
...
def test_commit_with_staged_changes_yes(tmp_git_repo: Path, monkeypatch, mlx_stub: MLXStub) -> None:
    import subprocess
...
def test_refactor_diff_applied(tmp_git_repo: Path, monkeypatch, mlx_stub: MLXStub) -> None:
    target = tmp_git_repo / "file.txt"
    target.write_text("hello\n")
    monkeypatch.chdir(tmp_git_repo)
    mlx_stub.response = "--- a/file.txt\n+++ b/file.txt\n@@ -1 +1 @@\n-hello\n+HELLO\n"
    runner = CliRunner()
...
def test_refactor_rejected_keeps_file(tmp_git_repo: Path, monkeypatch, mlx_stub: MLXStub) -> None:
    target = tmp_git_repo / "file.txt"
    target.write_text("hello\n")
    monkeypatch.chdir(tmp_git_repo)
    mlx_stub.response = "--- a/file.txt\n+++ b/file.txt\n@@ -1 +1 @@\n-hello\n+HELLO\n"
...
def test_analyze_directory(tmp_path: Path, mlx_stub: MLXStub) -> None:
    (tmp_path / "a.py").write_text("x = 1\n")
    mlx_stub.response = "## 구조\n분석 결과"
    runner = CliRunner()
    res = runner.invoke(cli, ["analyze", str(tmp_path)])
    assert res.exit_code == 0, res.output


def test_pr_creates_with_gh_missing(tmp_git_repo: Path, monkeypatch, mlx_stub: MLXStub) -> None:
    import subprocess
